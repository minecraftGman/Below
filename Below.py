...
class Below():
    ...
    hidedText = 'GG! You deobfuscated the code! obfuscated by Below'
    exceptionCode = 'input("Don\'t try to skid Impostor obfuscation !");exit(0)'

    infos = {
        '__author__': 'minecraftGman',
        '__madeBy__': 'minecraftGman',
    }
...
"""[1:-1].replace('MARSHALMODULE', Alt('__import__("marshal")'))
    ...
    def Obfuscate(code: str) -> str:
        ...
        runCode = f"""
if not ({Below.checkInfos}): {Below.exceptionCode}
exec({_code!r}, {program})
"""[1:-1]

        code__ = Below.serializer.dumps(compile(runCode, 'Below', 'exec'))
        infos_ = '\n'.join(f'{key} = {value!r}' for key, value in Below.infos.items())
...
{Below.gatewayClass}
{Below.interpreterClass}

Interpreter({code__!r},
            {base64.b64encode(Below.serializer.dumps(Below.RemoveLayers.__code__))!r},
            {Below.Alt('__import__("base64")')}, globals(),
            {base64.b16encode(Below.serializer.dumps(Below.Gateway.__code__))!r}
).Run()
"""[1:-1]
        return code_
...
def Main():
    ...
    with open(path, 'r', encoding = 'utf-8') as file:
        code = file.read()
    code = Below.Obfuscate(code)
    filename = os.path.basename(path).removesuffix('.py')
    with open(filename + '-Below.py', 'a+', encoding = 'utf-8') as file:
        file.write(code)
    Write.Print(f'[!] Successfully obfuscated !', Col.green, interval)
    input()

if __name__ == '__main__':
    System.Title('Below - by minecraftGman')
    System.Size(200, 50)
    Intro()
